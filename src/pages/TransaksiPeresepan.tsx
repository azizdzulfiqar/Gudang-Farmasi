...
import ClinicalCheck from '@/components/ClinicalCheck';
...
export default function TransaksiPeresepan() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-primary rounded-2xl p-6 flex flex-col justify-center relative overflow-hidden shadow-xl shadow-primary/20">
                  ...
                  {formData.items.length >= 2 && (
                    <Button 
                      onClick={() => setIsClinicalCheckOpen(true)}
                      variant="outline" 
                      className="mt-3 w-full bg-white/10 border-white/20 text-white hover:bg-white/20 h-10 rounded-xl gap-2 font-bold"
                    >
                      <Brain size={16} /> AI Clinical Check
                    </Button>
                  )}
                </div>
              </div>
      ...
      <ClinicalCheck 
        isOpen={isClinicalCheckOpen} 
        onOpenChange={setIsClinicalCheckOpen} 
